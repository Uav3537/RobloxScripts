	...
	task.spawn(function()
		...
		if not callback then
			if mainExists then
				...
				log("⏺️ " .. name .. " • Exists but behavior is not tested");
			elseif optional then
				optionalMissing += 1;
				table.insert(list_optional_missing, name);
				log("⏺️ " .. name .. " • Optional feature not provided");
			else
				fails += 1;
				missing += 1;
				table.insert(list_missing, name);
				log("⛔ " .. name .. " • Global is missing in this environment");
			end;
		elseif not mainExists then
			...
			if success then
				...
				log("✅ " .. name .. (message and " • " .. message or ""));
			else
				fails += 1;
				table.insert(list_fail, name .. " • " .. tostring(message));
				log("⛔ " .. name .. " failed: " .. tostring(message));
			end;
		end;
		...
		if #undefinedAliases > 0 then
			...
			log("⚠️ Missing alias(es) for " .. name .. ": " .. table.concat(undefinedAliases, ", "));
		end;
		running -= 1;
	end);
end;
log("\nUNC Environment Check");
log("✅ - Pass (exists and behavior works)");
log("⛔ - Fail (missing or behavior broken)");
log("⏺️ - Exists but not behavior-tested / optional missing");
log("⚠️ - Missing aliases\n");
...
task.defer(function()
	...
	log("✅ Passes: " .. passes);
	log("⛔ Fails: " .. fails);
	log("⏺️ Exists but untested: " .. existsOnly);
	log("❌ Missing required globals: " .. missing);
	log("⏺️ Missing optional globals: " .. optionalMissing);
	log("⚠️ Globals with missing aliases: " .. undefined);
	log("Tested success rate: " .. rate .. "% (" .. outOf .. ")\n");
    task.defer(function()
        repeat task.wait() until running == 0

        local final = table.concat(report, "\n")

        setclipboard(final) -- or toclipboard(final)

        print("📋 UNC report copied to clipboard!")
    end)
	local verdict;
	...
	if #list_missing > 0 then
		log("❌ Completely missing required globals:");
		log("  " .. table.concat(list_missing, ", "));
		log("");
	end;
	if #list_optional_missing > 0 then
		log("⏺️ Missing optional (executor-specific) globals:");
		log("  " .. table.concat(list_optional_missing, ", "));
		log("");
	end;
end);
local function runRcon()
	...
	log("UNC rconsole tests: " .. lp .. " passes, " .. lf .. " fails");
	local testedTotal = passes + fails;
	local rate = testedTotal > 0 and math.round(passes / testedTotal * 100) or 0;
	local outOf = passes .. " out of " .. testedTotal;
